...
function PanelPrincipal() {
  ...
  // Helpers to read and normalize auth info from sessionStorage
  const getOwnerEmail = () => {
    const saved = sessionStorage.getItem('user')
    if (!saved) return null
    try {
      const obj = JSON.parse(saved)
      const raw = (obj && (obj.email || obj.username || obj.name)) || ''
      const email = String(raw).trim()
      if (!email) return null
      const low = email.toLowerCase()
      if (low === 'none' || low === 'null') return null
      // debug: show normalized owner email
      // console.debug('[PanelPrincipal] ownerEmail normalized ->', email)
      return email
    } catch (e) {
      return null
    }
  ...
  const getTokenContract = () => {
    const raw = sessionStorage.getItem('tokenContract') || ''
    const token = String(raw).trim()
    if (!token) return null
    const low = token.toLowerCase()
    if (low === 'none' || low === 'null') return null
    // debug: show normalized token (do not log in production)
    // console.debug('[PanelPrincipal] tokenContract normalized ->', token)
    return token
  }
                  ...
                  {microservices.length === 0 ? (
                    ...
                    microservices.map((microservice) => (
                      <tr key={microservice.id}>
                        ...
                        <td >{(microservice.status === 'running' && microservice.last_started) ? formatDuration(Date.now() - Date.parse(microservice.last_started)) : '-'}</td>
