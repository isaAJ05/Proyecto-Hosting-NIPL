...
function PanelPrincipal() {
  ...
  useEffect(() => {
    ...
    let tokenContract = sessionStorage.getItem('tokenContract') || null
    sessionStorage.setItem("usernameFiltrado", username)
    sessionStorage.setItem("tokenFiltrado", tokenContract)
    const ownerEmail = username
    const headers = {}
    if (ownerEmail) headers['X-Owner-Email'] = username
    if (tokenContract) headers['X-Token-Contract'] = tokenConttokenContractract
