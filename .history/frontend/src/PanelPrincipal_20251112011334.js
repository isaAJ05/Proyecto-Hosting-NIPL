                ...
                <tbody>
                  {microservices.length === 0 ? (
                    <tr>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, color: lightTheme ? '#444' : '#ccc' }}>
                        <img
                          src="https://cdn-icons-png.flaticon.com/512/4076/4076500.png"
                          alt="Sin proyectos"
                          style={{ width: 100, opacity: 0.8 }}
                        />
                        <h3 style={{ margin: 0 }}>No tienes proyectos aún</h3>
                        <p style={{ maxWidth: 400, opacity: 0.8 }}>
                          Crea tu primer proyecto para empezar a organizar tus ideas y datos.
                        </p>
                        <button
                          style={{
                            background: lightTheme ? '#0d6efd' : '#2563eb',
                            color: 'white',
                            border: 'none',
                            borderRadius: 8,
                            padding: '10px 20px',
                          ...
                          onMouseOver={(e) => (e.currentTarget.style.background = lightTheme ? '#0b5ed7' : '#1d4ed8')}
                          onMouseOut={(e) => (e.currentTarget.style.background = lightTheme ? '#0d6efd' : '#2563eb')}
                        >
                          + Nuevo proyecto
                        </button>
                      </div>
                    ...
                    microservices.map((microservice) => (
                      <tr key={microservice.id}>
                        ...
                        <td>{microservice.subdomain || microservice.repo || '-'}</td>
                        ...
                        <td style={{ fontSize: 12 }}>{microservice.created_at ? new Date(microservice.created_at).toLocaleString() : '-'}</td>
                        <td style={{ fontSize: 12 }}>{(microservice.status === 'running' && microservice.last_started) ? formatDuration(Date.now() - Date.parse(microservice.last_started)) : '-'}</td>
                        <td style={{ display: "flex", gap: 6, alignItems: 'center' }}>
                        <button
                          className="action-btn"
                          title="Abrir"
                          onClick={() => {
                            const url = microservice.subdomain ? `http://${microservice.subdomain}` : null
