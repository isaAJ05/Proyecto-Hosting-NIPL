...
def deploy_from_repo():
    ...
    ok, result = create_project_from_repo(repo_url, name, user=user, subdomain=subdomain, cpu=cpu, memory=memory, rate_limit_per_minute=rate_limit_per_minute, owner_email=owner_email, owner_token_contract=owner_token_contract)
    if not ok:
...
def forward_auth(project_id):
    ...
    if p.get('status') != 'running':
        start_ok, start_res = start_project(project_id)
        # If start failed, return 503 to indicate service unavailable
        if not start_ok:
            return jsonify({'error': 'starting container failed', 'details': start_res}), 503
        # update p reference after starting
        db = load_db()
        p = db.get(project_id)
    # touch last_access
    touch_project(project_id)
    # Return 200; Traefik will forward the request to container
