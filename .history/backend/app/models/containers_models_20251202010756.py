...
def build_and_run(project_id, project_dir, image_tag, subdomain, cpu=None, memory=None, container_name=None, rate_limit_per_minute=None, enable_forward_auth=True):
    ...
    if subdomain:
        ...
        if enable_forward_auth:
            # Use the backend service name so Traefik (in the same Docker network)
            # can call forward_auth directly. This avoids relying on
            # host.docker.internal which may not be available in some setups.
            forward_url = f'http://proyecto_backend:8000/projects/{project_id}/forward_auth'
